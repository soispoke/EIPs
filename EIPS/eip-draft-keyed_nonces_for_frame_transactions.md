...
author: Thomas Thiery (@soispoke) <thomas.thiery@ethereum.org>, Toni Wahrstätter (@nerolation) <toni.wahrstaetter@ethereum.org>
discussions-to: TBD
status: Draft
...
At finalization, the literal `TBD` in the `DOMAIN_PREIMAGE` below MUST be replaced with the assigned EIP number in decimal, without zero-padding.
...
```text
DOMAIN_PREIMAGE = "EIP-TBD:KEYED_NONCE_DOMAIN"
...
At finalization, the final EIP MUST publish the resulting 32-byte `DOMAIN` value.
